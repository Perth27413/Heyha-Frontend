...
<main id="paginateMain">
  <div id="paginateButtonBox">
    <div class={`paginate-button ${isFirst || currentPage === 0 ? 'disable' : ''}`} on:click={() => !isFirst ? setPage(1) : undefined}>{'<<'}</div>
    <div class={`paginate-button ${isFirst || currentPage === 0 ? 'disable' : ''}`} on:click={() => !isFirst ? setPage(currentPage - 1) : undefined}>{'<'}</div>
    ...
    <div class={`paginate-button ${isLast || currentPage === 0 ? 'disable' : ''}`} on:click={() => !isLast ? setPage(currentPage + 1) : undefined}>{'>'}</div>
    <div class={`paginate-button ${isLast || currentPage === 0 ? 'disable' : ''}`} on:click={() => !isLast ? setPage(totalPage) : undefined}>{'>>'}</div>
  </div>
</main>
